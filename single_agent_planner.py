...
def build_constraint_table(constraints, agent):
    ...
    for constraint in constraints:
        if constraint['agent'] == agent:
            if constraint['timestep'] in table_constraints.keys():
                table_constraints[constraint['timestep']].append(constraint['loc'])
            else:
                table_constraints[constraint['timestep']] = [constraint['loc']]
...
def is_constrained(curr_loc, next_loc, next_time, constraint_table):
    ...
    if next_time in constraint_table:
        for c in constraint_table[next_time]:
            if c == [curr_loc, next_loc] or c == [next_loc]:
                return True
    return False
...
def a_star(my_map, start_loc, goal_loc, h_values, agent, constraints):
    ...
    while len(open_list) > 0:
        ...
        for dir in range(5):
            ...
            #checking boundaries (len(my_map)) should be 5
            if child_loc[0] < 0 or child_loc[0] >= len(my_map):
                continue

            if child_loc[1] < 0 or child_loc[1] >= len(my_map[0]):
                continue
